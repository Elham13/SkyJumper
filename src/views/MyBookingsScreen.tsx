...
const MyBookingsScreen = () => {
  ...
  const renderItem = ({item}:{item: BookingProps}) => <BookingItem id={item.id} status={item.status} name={item.name} price={item.price} time={item.time}/>
    ...
    <SafeAreaView>
      <StyleView className='ww-full h-full p-2' level='1'>
        ...
        <StyleView className='w-full h-[92%] p-2' level='2'>
          <StyleFlatList data={bookingData} renderItem={renderItem} keyExtractor={(item) => item.id} />
        </StyleView>
    </StyleView>
    </SafeAreaView>
